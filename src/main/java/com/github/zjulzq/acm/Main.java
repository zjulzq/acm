...
public class Main {
    ...
    private static void calcLastRow(int[] first, int[] second) {
        for (int i = 0; i < first.length; i++) {
            if (i == 0) {
                diff.add(maxAbs(second[i] - first[i], second[i] - first[i + 1], second[i] - second[i + 1]));
            } else if (i == first.length - 1) {
                diff.add(maxAbs(second[i] - first[i - 1], second[i] - first[i], second[i] - second[i - 1]));
            } else {
                diff.add(maxAbs(second[i] - first[i - 1], second[i] - first[i], second[i] - first[i + 1], second[i] - second[i - 1], second[i] - second[i + 1]));
            }
        }
    }
